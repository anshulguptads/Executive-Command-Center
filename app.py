...
total_revenue = sales_f["Sales_Revenue"].sum()
avg_basket = sales_f["Basket_Size"].mean()
avg_price = sales_f["Unit_Price"].mean()
total_units = sales_f["Units_Sold"].sum()
avg_footfall = sales_f["Footfall"].mean()
conv_proxy = ((sales_f["Web_Orders"] + sales_f["Mobile_Orders"]).sum() / (sales_f["Footfall"].sum() + 1)) * 100
